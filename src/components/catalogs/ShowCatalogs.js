...
import { fetchCatalogs, deleteCatalog, clearCatalogsErrors } from '../../actions';
...
const propTypes = {
  ...
  catalogsList: PropTypes.object.isRequired,
  catalogDelete: PropTypes.bool,
  onCatalogDisable: PropTypes.func,
};
...
class ShowCatalogs extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.catalogDelete && this.state.delete) {
      this.setState({
        delete: false,
        alertOpen: true,
        alertMessage: 'Catalog deleted!',
      });
      this.handleClose();
    }
  ...
  handleDelete() {
    ...
    this.props.dispatch(deleteCatalog(this.state.deleteId));
  }
  handleDisable(item) {
    const data = item;
    data.disabled = !item.disabled;
    this.props.onCatalogDisable(data);
  }
  alertHandleClose() {
    this.setState({
      alertMessage: '',
  ...
  renderRowsTable() {
    return this.props.catalogsList.results.map(item =>
      <TableRow key={item.id}>
        ...
        <TableRowColumn key={item.id}>
          <ActionButtons
            onEdit={`/catalogs/${item.id}?page=1`}
    ...
    return (
      <div>
        <DialogModal
          ...
          dialogTitle="Delete Catalog"
          dialogMessage="Do you really want to delete this catalog?"
...
ShowCatalogs.childContextTypes = {
  muiTheme: PropTypes.object.isRequired,
};

ShowCatalogs.propTypes = propTypes;

function mapStateToProps(state) {
  return {
    catalogDelete: state.catalogs.catalogDelete,
    error: state.catalogs.error,
  };
}

export default connect(mapStateToProps)(ShowCatalogs);
